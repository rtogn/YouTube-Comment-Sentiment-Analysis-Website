...
### 1.4 Scope
The youtube sentiment analyser is meant for people who would like to filter out negative youtube comments before reading them. The app will have a search bar similar to Google's. The bar will put to use the Youtube API in order to display videos when searched for. Once a video is selected it will allow the user to filter out negative comments or keywords based on widgets. 

There are already a couple services in existence that offer to filter things based on sentiment, but most of them cost money. Our service will be free. 

### 1.5 Definitions and Acronyms

1.5.1 This document will use the following acronym conventions:
-	YCSA - Youtube Comment Sentiment Analysis. The web application being discussed in this document.
-	NLTK - Natural Language Toolkit (https://www.nltk.org/). The library being used for the base textual sentiment analysis.
...
1)	Users of the Youtube site often encounter comments that are overly negative or offensive, and there is no built-in way to screen these comments. 
2)	Metrics such as view count and subscriptions are not necessarily indicative of the overall opinion of a video. Additionally, the recent removal of the ‘dislike’ count by Google leaves users with only likes to gauge the content of a video and its reception. This does not allow for negative views on a video to be considered in their selection. 
...
1.	When a first-time user opens YCSA, they will be taken to a page asking them to either sign up or log in through in already existing Google account.
2.	The first-time user will click on the sign-up button, and enter their Google account.
...
4.	Once clicked, it will display all of the videos related to the keyword the user searched.
...
1.	On the homepage, you will also find on the left side of the page the saved comments button.
2.	When the user clicks on this button, they will get a display of all the comments that are saved in the previous log ins.
3.	The user can also unsave their previous comments on this page.
4.	If the user hovers over an individual comment, the more icon will appear.
5.	In the 'more' icon, one of the options that will appear is “unsave”. If the users clicks it, the comment will be unsaved.
...
1.	A user if logged in will be able to select the “Subscriptions” page.
2.	Upon loading, the Subscrptions page will display a grid of videos recently released by accounts the user is subscribed to.
...
6.	The user will be directed to the player view upon selection of any given video.
...
#### 3.2.1 After logging in, user will be brought to an empty home page with no displayed search results:
![home_page](images/home_page.png)
#### 3.2.2 User enters keywords into the search bar and selects the search button which will refresh the page to display results in a grid:
...
2.      Pages and searches should always load with a mere click and should do so in the adequate time frame of under 3 seconds.
